...
void printEquationsSystem(const std::string & text, Matrix & printingMatrix){
	std::cout << text << "{" << std::endl;
	for (unsigned int iIndex = 0; iIndex < printingMatrix.size1(); ++iIndex){
		unsigned char printedPrev = 0;
		for (unsigned int jIndex = 0; jIndex < printingMatrix.size2(); ++jIndex){
			if (jIndex && printedPrev){
				if (printingMatrix(iIndex, jIndex) > 0){
					std::cout << " + ";
				}
			...
			if (printingMatrix(iIndex, jIndex) > 0 || (!printedPrev && printingMatrix(iIndex, jIndex))){ // ( ... && ... ) - added for removing clang warning
				std::cout << printingMatrix(iIndex, jIndex) << "*X" << jIndex + 1;// << " ";
				printedPrev = ~0;
			}
			else if (printingMatrix(iIndex, jIndex) < 0){									
				std::cout << -printingMatrix(iIndex, jIndex) << "*X" << jIndex + 1;// << " ";
...
int main(int argc, char** argv){
    try{
		...
		Vector vectorB(4, vectorB_values);
...
#endif
		printEquationsSystem("System of equations: ", matrixA);
